...
def run_post_training_code(user_id):
    ...
    pipe.to("cuda")
    pipe.load_lora_weights(f"user{user_id}", weight_name="pytorch_lora_weights.safetensors")

    prompt = f"A portrait of user{user_id} wearing a professional business suit in an professional office"
...
def save_generated_images(images, user_id):
    generated_dir = f'media/generated/user_{user_id}/'
